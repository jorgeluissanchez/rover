...
export function AreaChartTemp({data}: {data: any}) {
  function calcularPorcentajeCrecimiento() {
    const n = data.length;
    if (n < 2) {
        return 0;
    }
    const ultimoValor = data[n - 1].Temperature;
    const valorAnterior = data[n - 2].Temperature;
  ...
  return (
    ...
    <Card className="flex items-center justify-between px-4 py-3.5 gap-2 hover:cursor-pointer hover:bg-slate-200">
      <div className="flex items-center space-x-2.5">
        <p className="text-tremor-content-strong dark:text-dark-tremor-content-strong font-medium">TEMP</p>
      </div>
